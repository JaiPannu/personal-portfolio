import './Header.css';
import '../global.css';
import useLiquidGlassHover from '../../hooks/useLiquidGlassHover';

const Header = () => {
  const navRef = useLiquidGlassHover();

  return (
    <header className="header">
      <a href="#home" className="header-logo-link">
        <img src="/JSPlogoNBG.png" alt="Home" className="header-logo-img" />
      </a>
      <nav ref={navRef} className="nav liquid-glass liquid-glass-rounded-xl">
        <div className="liquid-glass-specular"></div>
        <a href="https://drive.google.com/file/d/1zpSg9ygdsdcZcUHEZ7FiEUxbSWNy_9HA/view?usp=sharing" target="_blank"className="nav-logo">My Resume</a>
        <a href="#projects" className="nav-link">Projects</a>
        <a href="#about" className="nav-link">About</a>
        <a href="#skills" className="nav-link">Skills</a>
        <a href="#experience" className="nav-link">Experience</a>
        <a href="#contact" className="nav-link">Contact</a>
      </nav>
    </header>
  );
};

export default Header;
